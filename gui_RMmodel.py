...
# メイン実行の関数
def Calc_Munu(Attendfile,Needfile,Attendsheet,Needsheet,K,W,bikou,slack_check):
...
# 実行ボタン
calc_button = tk.Button(root,text="実行",command=lambda:Calc_Munu(Attend_path.get(), Need_path.get(), Attend_sheet.get(), Need_sheet.get(), entry_K.get(), entry_W.get(), entry_bikou.get(), bln.get()))
